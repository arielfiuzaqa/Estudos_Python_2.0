...
nome_aba_semanal = 'SAC\'s Semanal'  # Usado para a aba temporária
nome_aba_baixa = 'PARA DAR BAIXA'  # Usado para a aba temporária da BAIXA

# Caminho da planilha original
caminho_planilha_original = pasta_downloads + "Planilha.xlsx"
...
nome_nova_planilha = f"{pasta_downloads}TAB PLANILHA DE PUBLICAÇÃO SEMANAL - {date_tomorrow}.xlsx"
...
# Salva o DataFrame filtrado usando pandas na aba semanal
...
for row in ws.iter_rows(min_row=2, max_row=ws.max_row, min_col=1, max_col=13):
    for cell in row:
        ...
        if cell.column_letter == 'A' and cell.value == "Publicação Semanal":
